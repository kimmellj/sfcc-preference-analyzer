 ...
 */
function saveCSV(name, preferencesCSV) {
    stringify(preferencesCSV, function (err, output) {
        fs.writeFile(path.join(__dirname, `report-${name}.csv`), output, 'utf8', function (err) {
            if (err) {
                log.error('Some error occured - file either not saved or corrupted file saved.');
            }
        });
    });
}
 ...
 */
async function saveXLS(name, preferencesCSV) {
    ...
    await workbook.xlsx.writeFile(path.join(__dirname, `report-${name}.xlsx`));
}
